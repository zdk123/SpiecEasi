...
**Note**: On Windows systems, `mc.cores > 1` is not supported by default. For Windows users, we recommend running in serial mode or using batch mode with `conffile='snow'`.
...
### Option 1: Use batch mode with snow

For Windows users who want parallel processing, use batch mode with the
`snow` configuration. See the [Batch Mode](#batch-mode) section below for
a full example, and use `conffile='snow'` instead of `conffile='parallel'`.
...
3. **For large datasets**: Use batch mode with `conffile='snow'`
4. **For HPC clusters**: Use batch mode with appropriate config files
