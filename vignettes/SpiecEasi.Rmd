...
```{r, eval=FALSE}
# Basic multicore processing doesn't work on Windows
ncores <- if(.Platform$OS.type == "windows") 1 else 4

se1.mb.amgut <- spiec.easi(amgut1.filt, method='mb', lambda.min.ratio=1e-2,
                          nlambda=20, pulsar.params=list(rep.num=20, ncores=ncores))
...
## Parallel multicore ##
ncores <- if(.Platform$OS.type == "windows") 1 else 4
pargs2 <- list(rep.num=50, seed=10010, ncores=ncores)
t2 <- system.time(
